/**
 * AeroTrace AI — Map Layer Renderer Module
 * ─────────────────────────────────────────
  ...
  <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;min-width:180px;">
    ...
    <div style="font-size:11px;color:#94a3b8;line-height:1.5;">
      ...
      <div style="margin-top:4px;padding-top:4px;border-top:1px solid rgba(255,255,255,0.08);">
        ...
        · <span style="color:#cbd5e1;text-transform:uppercase;font-size:10px;">${station.dominant_pollutant}</span>
      </div>
    </div>
  </div>
`;
...
// ═══════════════════════════════════════════════════════════════════════════════
// MAP RENDERER — Public API
// ═══════════════════════════════════════════════════════════════════════════════

export const MapRenderer = {

  // ─── 1. CORE MAP INIT (Vanilla Leaflet — standalone mode) ───────────────
  /**
  ...
  // ─── 2. STATION GRID LAYER ──────────────────────────────────────────────
  /**
   * Fetch stations from API and render as pulsing circle markers.
   * AQI > 200 → crimson pulse animation. Otherwise → calm blue glow.
   *
  ...
  async renderStations(map, onStationSelect) {
    ...
    try {
      ...
      stations.forEach((station) => {
        // Backend coordinates: [lon, lat] (GeoJSON) → Leaflet [lat, lon]
  ...
  // ─── 3. POLLUTION SOURCE LAYER ──────────────────────────────────────────
  /**
   * Fetch all pollution sources and render with origin-based differentiation.
   * curated → solid red marker. osm → dashed amber outline.
   *
  ...
  /** Swap GeoJSON [lon, lat] → Leaflet [lat, lon]. */
  geoToLatLng(coords) {
